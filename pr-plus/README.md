...
- GPT 추천일과 달라도 오늘 운동으로 시작하고, 원래 예정일·실제 수행일·지연/당김 일수를 다음 상담에 전달
- Warm-up / Working / Back-off 세트 역할, 구조화된 통증·이상 신호와 중량 표시 기준
- 운동 중 좌우 스와이프 카드 덱, 완료 시 날짜 카드팩 수집 애니메이션
- 기록의 날짜별 카드팩 보관함과 기간 통계를 압축한 금속 프레스 대시보드
...
PR+ v0.4.0의 IndexedDB 버전은 `3`, 백업/저장 데이터 schema는 `6`입니다. v0.4.0은 표시 방식만 바꾸므로 데이터 migration은 추가하지 않습니다. 기존 데이터는 `v1 → v2 → v3 → v4 → v5 → v6` 인접 migration을 순서대로 거치며 운동·루틴·세션 기록은 그대로 유지됩니다. 상세 규칙은 [`MIGRATIONS.md`](./MIGRATIONS.md)를 참고하세요.
...
- GPT 추천 세션은 예정일이 지나거나 아직 오지 않았더라도 오늘 운동으로 시작할 수 있습니다.
- 예정일과 다른 날 시작하면 변경 방향과 일수를 확인한 뒤 세션에 `scheduledTrainingDate`, `actualTrainingDate`, `scheduleOffsetDays`, `scheduleStatus`를 저장합니다.
- 루틴 카드와 오늘 운동 화면에서 원래 예정일과 실제 수행일을 함께 표시합니다.
- 다음 Coach Protocol 요청에는 `scheduleChanges`가 포함되어 GPT가 “원래 며칠 운동이었지만 며칠 미뤄 수행했다”는 사실을 일정과 회복 판단에 반영할 수 있습니다.

## v0.4.0 카드 덱 인터페이스

- 오늘 운동은 한 장씩 집중하는 좌우 스와이프 덱으로 표시하며 양옆 카드 층과 잔여 장수로 전체 순서를 파악할 수 있습니다.
- 카드 아래의 화살표와 위치 점으로도 이동할 수 있고, 입력칸과 버튼 위에서는 스와이프를 가로채지 않습니다.
- 운동 완료 시 모든 운동 카드가 오늘 날짜 카드팩으로 모이는 애니메이션이 재생됩니다. 모션 감소 설정에서는 애니메이션을 생략합니다.
- 기록 화면은 날짜별 카드팩 보관함으로 표시하며 팩을 열면 그날의 운동 카드가 순서대로 펼쳐집니다.
- 선택 기간의 성장 지표와 그래프는 해당 카드팩들을 압축한 금속 프레스 강판으로 표현합니다.
- 이 릴리스는 UI만 변경하므로 IndexedDB `3`, 데이터 schema `6`, Coach Protocol `1.1`을 유지합니다.
